...
import java.util.*;
   ...
   public class IDACSServerListener extends Thread
   {
      ...
      private int initialThreadPriority = 7;
         ...
         while((ParentConnection.ListenerKeepRunning == true) && (SystemError == false))
         {
            ...
            try
            {
               for(int index = 0; index < ParentConnection.highestActiveSessionIndex; index++)
               {
                  ...
                  {
					 System.out.println("cancelCurrentFlightTimer : " + index);
                     ParentConnection.ActiveSessions[index].flightActive = false;
                     ParentConnection.ActiveSessions[index].flightComplete = false;
					 //ParentConnection.ActiveSessions[index].ActiveFlightInitialSequenceNumber = 0;
            ...
            {				
               ...
               for(int index = 0; index < ParentConnection.highestActiveSessionIndex; index++)
               {
            	...
            	System.out.println("current Session index : " + currentSessionIndex + " remote IP is : " + Client + " Flight Initial : " + flightInitialSequenceNumber);
               ...
               IDACSSession currentSession = ParentConnection.ActiveSessions[currentSessionIndex];//xiao
				...
				if(currentSession != null)
					System.out.println("session " + currentSessionIndex + " is not null");
				else
					System.out.println("session " + currentSessionIndex + " is null");
               ...
               {	
				...
				if(data != null){
					...
					// try {
						// FileOutputStream fos = new FileOutputStream(new File("IDACSServerListenrer.txt"),true);
						// fos.write((sequenceNumber+"").getBytes());
						// fos.write("\r\n".getBytes());
						// fos.write(data);
						// fos.write("\r\n".getBytes());
						// fos.close();
                  ...
                  if((currentSession.ReceivedQueueRightEdge + 1) % currentSession.ReceivedQueue.length != currentSession.ReceivedQueueLeftEdge)
                  {
					 ...
					 //System.out.println("RIGHT EDGE : " + currentSession.ReceivedQueueRightEdge);
                  }
				  
                  else
                  {
					System.out.println("No room in current session drop packet");
               ...
               else if((type == IDACSCommon.DATA_MESSAGE_TYPE) && (flightSize > 1))
               {
				...
				System.out.println("Flight Active : " + currentSession.flightActive + " currentSession.ActiveFlightInitialSequenceNumber : " + currentSession.ActiveFlightInitialSequenceNumber + "  dataoffset : " + dataOffset);
                  ...
                  {
				    System.out.println("find active flight related with");
