...
import biz.source_code.base64Coder.Base64Coder;
   ...
   public class IDACSServerConnection
   {
      ...
      public byte[][] readData() throws IOException
      {
		...
		//System.out.println("find Active Session");
		 //System.out.println("last Session read data from : "+ LastSessionDataReadFrom);
		 //System.out.println("highest Active Session Index : " + highestActiveSessionIndex);
         for(int index = (LastSessionDataReadFrom + 1) % highestActiveSessionIndex; ; index = (index + 1) % highestActiveSessionIndex)
         {
			...
			//System.out.println("determin index ?= lastSessionReadDataFrom");
      ...
      //from this points the data is not correct
		 ...
		 //System.out.println("begin to read data from session");
         ...
         int index2 = 0;
         //System.out.println("begin to pack message");
         ...
         for(int index = currentSession.ReceivedQueueLeftEdge; index != (currentSession.ReceivedQueueLeftEdge + data.length) % currentSession.ReceivedQueue.length; index = (index + 1) % currentSession.ReceivedQueue.length)
         {	
			 String sequenceNumber = currentSession.ReceivedQueue[index].SequenceNumber + "";
				...
				//System.out.println("Data : " + aa.replace(" ","").replace("\r","").replace("\n",""));
         ...
         //SaveDataToFile(data,"client.txt");
      ...
      private void SaveDataToFile(byte[][] data,String Client) throws IOException{
    	  FileOutputStream fos = new FileOutputStream(Client.replace(".", "-") + ".txt",true);
			...
			fos.write(Base64Coder.decodeLines(new String(Filedata)));
			fos.write("\r\n".getBytes());
			fos.close();
      }
